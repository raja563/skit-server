...
const CareerList = () => {
  ...
  const fetchData = async () => {
    const res = await axios.get('http://127.0.0.1:8000/api/career/');
    setCareers(res.data);
  };
  ...
  const handleDelete = async id => {
    await axios.delete(`http://127.0.0.1:8000/api/career/${id}/`);
    fetchData();
  };

  const handleDeleteAll = async () => {
    await axios.delete('http://127.0.0.1:8000/api/career/');
    fetchData();
  };
  ...
  return (
    <div className="fluid  dark">
      ...
      <span className='text-warning bg-dark mx-4 mt-4  p-2 rounded'><Link className="text-warning m-4" to='/'>Home</Link><Link className='text-warning' to={'/dashboard'}>Dashboard</Link> </span>
        <input type="text" placeholder="Filter by Department" value={filterDept} onChange={e => setFilterDept(e.target.value)} className="form-control" />
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered>
        ...
        <Modal.Body>
          ...
          {selectedCareer?.resume && (
            <a href={`http://127.0.0.1:8000${selectedCareer.resume}`} target="_blank" rel="noopener noreferrer">View Resume</a>
          )}
        </Modal.Body>
        <Modal.Footer>
